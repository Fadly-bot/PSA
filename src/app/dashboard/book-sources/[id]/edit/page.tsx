  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = (await res.json()) as BookSource;
        if (!cancelled) {
          setName(data.name ?? '');
  ...
  if (notFound) {
    return (
      <div style={{ maxWidth: 720 }}>
        <div style={{ marginBottom: 16 }}><Link href="/dashboard/book-sources" className="btn secondary">Kembali</Link></div>
        <div className="card" style={{ padding: 24 }}>Data sumber buku tidak ditemukan.</div>
      </div>
    );
  }
