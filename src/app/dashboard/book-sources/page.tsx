...
export default function BookSourcesPage() {
  const [items, setItems] = useState<BookSource[]>([]);
  ...
  const load = async (pageNum: number, query?: string) => {
    ...
    setError(null);
    try {
      const url = new URL('/api/book-sources', window.location.origin);
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <Link href="/dashboard/book-sources/new" className="btn">+ Tambah Sumber</Link>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'auto' }}>
        ...
        {error ? (
          ...
          <div style={{ padding: 24, color: 'var(--muted)' }}>Belum ada data sumber buku.</div>
        ) : (
